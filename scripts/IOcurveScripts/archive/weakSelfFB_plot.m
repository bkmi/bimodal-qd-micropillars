...
%% generate data
% Turn this on if you want to generate data. I already have data so I won't
...
for j = 1 % :numel(relFBamps) % NO FB
    ...
    % this feature lets you split the graph at a certain point.
    cutit = numel(flipedJ)-18;
    % strong grey
...
for j = 2 % :numel(relFBamps)
    ...
    % this feature lets you split the graph at a certain point.
    cutit = numel(flipedJ); % 99; % I don't want to cut it, so I set this to the max.
    % strong grey
    strongPlotsSweepDown{j} = plot( ...
        flipedJ(1:cutit), ...
        strongFinalIntensityDownSweep(1:cutit) * ySclFactor, ...
...
for j = 3 % :numel(relFBamps)
    ...
    % between 14 and 15
    % this feature lets you split the graph at a certain point.
    cutit = numel(flipedJ); % 14; % I don't want to cut it, so I set this to the max.
    % strong grey
    strongPlotsSweepDown{j} = plot( ...
        flipedJ(1:cutit), ...
        strongFinalIntensityDownSweep(1:cutit) * ySclFactor, ...
