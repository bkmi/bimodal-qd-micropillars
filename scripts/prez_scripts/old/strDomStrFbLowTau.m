...
setup_params_nonDim_CnstCplRatio(...
    ...
    'tau_fb', 0.8, ...
    'datadir_specific', ...
    strcat(data_directory(), '/strDomStrFbLowTau'))
...
%% Filling out the fold lines, outside fold lines.
...
save([master_options.datadir_specific,'horizBranch045.mat'], ...
    'horizBranch045');

% folds from horizBranch045

folds_horBrn045 = bifurFoldHopfMultiCreator( ...
    funcs, ...
    horizBranch045, ...
    param, ...
    horizBranch045.indFold, ...
    30, ...
    'step_bound_opt', step_bound_forFOLD);

branchplot = figure;

for i = 1:numel(folds_horBrn045)
    % Add each fold
    if isa(folds_horBrn045(i).error,'double') && folds_horBrn045(i).error == 0
        plot_branch(folds_horBrn045(i), param, ...
            'add_2_gcf', 1, 'color','r');
    end
end

save([master_options.datadir_specific,'folds_horBrn045.mat'], ...
    'folds_horBrn045');
%% inside fold lines??

horizBranch03 = pickAndSwitch(funcs, ...
    vertBranch085, ...
    param.feed_phase.index, ...
    450, ...
    param, ...
...
plot_branch(horizBranch03, param, ...
    'nunst_color', horizBranch03.nunst)

save([master_options.datadir_specific,'horizBranch03.mat'], ...
    'horizBranch03');

% folds from horizBranch03

folds_horBrn03 = bifurFoldHopfMultiCreator( ...
    funcs, ...
    horizBranch03, ...
    param, ...
    horizBranch03.indFold([1,4]), ...
    70, ...
    'step_bound_opt', step_bound_forFOLD);

branchplot = figure;

for i = 1:numel(folds_horBrn03)
    % Add each fold
    if isa(folds_horBrn03(i).error,'double') && folds_horBrn03(i).error == 0
        plot_branch(folds_horBrn03(i), param, ...
            'add_2_gcf', 1, 'color','r');
    end
end

save([master_options.datadir_specific,'folds_horBrn03.mat'], ...
    'folds_horBrn03');


%% Final bit of fold

horizBranch037 = pickAndSwitch(funcs, ...
    vertBranch085, ...
    param.feed_phase.index, ...
    450, ...
    param, ...
...
plot_branch(horizBranch037, param, ...
    'nunst_color', horizBranch037.nunst)

save([master_options.datadir_specific,'horizBranch037.mat'], ...
    'horizBranch037');
% Not sure this is a good one... I need the other fold and it gets
% reflected before that fold.
