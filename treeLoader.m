function [ fileData ] = treeLoader( varargin )
%Load data from the tree data directory.
...
p.addParameter('datadir_parent','../data_bimodal-qd-micropillars/zeroPhaseOffsetJ=560uA/')
...
if ~isdir(options.datadir_specific) ...
    ...
    % Since this is 'tree' we first pick the weakDom or strongDom
    ...
    % Round Two, pick the tau level.
    datadir_parent2 = strcat(options.datadir_parent,datadir_subfolder);
    
    directory = dir(datadir_parent2);
    sub_all_list = {directory.name};
    which_are_folders = [directory.isdir];
    subfolder_list = sub_all_list(which_are_folders);
    [selection,exit_box] = listdlg('PromptString','Select a folder:',...
                                            'SelectionMode','single',...
                                            'ListString',subfolder_list);
    datadir_subfolder2 = strcat(subfolder_list{selection},'/');
    if exit_box==0
        error('You did not choose a folder. Nothing was loaded')
    end
    
    
    loadDir = strcat(datadir_parent2,datadir_subfolder2);
else
